...
class WeatherCanvas {
  ...
  void update( boolean forceDraw ) {
    Boolean drawIt = false;
    Calendar currentTime = Calendar.getInstance();
    if ( currentTime.after( nextWeatherUpdateTime ) || forceDraw ) {
      nextWeatherUpdateTime = Calendar.getInstance();
      nextWeatherUpdateTime.add( Calendar.SECOND, secondsBetweenWeatherUpdates );
      if( !forceDraw ) { weather.FetchWeather(); }
      for ( int i = 0; i<numDayForecasts; i++ ) {
        dayForecast[i].update(forceDraw);
      }
      currentConditionsDisplay.update(forceDraw);
      drawIt = true;
    }
    if ( currentTime.after( nextTimeUpdateTime ) || forceDraw ) {
      nextTimeUpdateTime = nextMinute();
      dateDisplay.update(forceDraw);
      timeDisplay.update(forceDraw);
      drawIt = true;
    }
    if ( drawIt || forceDraw ) {
      drawCanvas();
  ...
  class TimeDisplay {
    ...
    void update( boolean forceDraw ) {
      ...
      if ( !timeString.equals(newTimeString ) || forceDraw ) {
  ...
  class DateDisplay {
    ...
    void update( boolean forceDraw ) {
      ...
      if ( !dateString.equals(newDateString ) || forceDraw ) {
  ...
  class CurrentConditionsDisplay {
    ...
    void update( boolean forceDraw ) {
      ...
      boolean b1 = td.update(forceDraw);
      boolean b2 = wd.update(forceDraw);
      if ( b1 || b2 || forceDraw ) {
        can.beginDraw();
        can.clear();
        if( drawBG ) {
  ...
  class WindDisplay {
    ...
    boolean update( boolean forceDraw ) {
      int newWindDir;
      int newWindSpeed;
      ...
      if ( newWindDir != windDir || newWindSpeed != windSpeed || forceDraw ) {
  ...
  class BasicDayForecast {
    ...
    boolean update( boolean forceDraw ) {
      ...
      if ( dayOfWeekChanged || dateChanged || hiTempChanged || loTempChanged || iconChanged || forceDraw ) {
